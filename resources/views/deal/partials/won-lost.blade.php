<form method="POST" action="{{ route('deal.update', ['id' => $deal->id]) }}" class="inline">
    ...
    @csrf
    @method('PUT')
    <x-primary-button x-data="{ show: false }" x-show="show || '{{ $type }}' === 'won'"
        ...
        <svg class="w-6 h-6 text-white-800 dark:text-gray" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
    ...
    <x-primary-button x-data="{ show: false }" x-show="show || '{{ $type }}' === 'lost'"
        ...
        <svg class="w-6 h-6 text-minus-800 dark:text-gray" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
            width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
            <path fill-rule="evenodd"
